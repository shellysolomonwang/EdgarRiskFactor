...
class TenKScraper:
    ...
    def scrape_method2(self, input_path, output_path):
        with open(input_path, 'rb') as input_file:
            ...
            # pattern = re.compile(r'ITEM\s1A\s*[\.|\/]\s+RISK FACTORS(.*?)ITEM\s1B\s*[\.|\/]\s*(UNRESOLVED STAFF COMMENTS|UNRESOLVED SEC STAFF COMMENTS)', re.IGNORECASE)
            # added '?' to match 'ITEM 1A' & 'ITEM 1B' without dot
            # pattern = re.compile(r'ITEM\s1A\s*[\.|\/]?\s+RISK FACTORS(.*?)ITEM\s1B\s*[\.|\/]?\s*(UNRESOLVED STAFF COMMENTS|UNRESOLVED SEC STAFF COMMENTS)', re.IGNORECASE)
            ...
            pattern_map = {'RRC': pattern_1, 'VLO': pattern_2, 'XEC': pattern_3, 'HES': pattern_4, 'OXY': pattern_5, 'RMD': pattern_6, 'LLY':pattern_6, '/LH': pattern_6, 'CAH':pattern_7,
            'WFC':pattern_8, 'USB': pattern_9, 'SYF':pattern_10, '/MS':pattern_11, 'LNC':pattern_6, 'HIG':pattern_6, 'l/C': pattern_12, 'AJG':pattern_6
            }
            pattern_list = [pattern]
            if input_path[-16:-13] in pattern_map:
                pattern_list.append(pattern_map[input_path[-16:-13]])
            rawText = None
...
if __name__=='__main__':
    # company_name = 
    # downloader = TenKDownloader(company_name, '20000101','20191101')
    # downloader.download()
    ...
    company_name = 'AFL'
    for file in glob.glob('./Financial/'+company_name+'/*.htm'):
        #print(file[-12:-3])
        scraper.scrape_method2(file, './Financial/'+company_name+'/'+file[-12:-3]+'txt') # makse sure ./data/txt exists
